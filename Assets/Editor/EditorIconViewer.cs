			...
			if (_showingStyles)
			{
				foreach (GUIStyle ss in GUI.skin.customStyles)
				{
					...
					draw.Draw = () =>
					{
						...
						GUILayout.BeginHorizontal();
						Rect rect = GUILayoutUtility.GetRect(inactiveText, GUI.skin.label, GUILayout.Width(width / 2));
						if (Event.current.type == EventType.Repaint)
						{
							thisStyle.Draw(rect, inactiveText, false, false, false, false);
						}
						rect = GUILayoutUtility.GetRect(activeText, GUI.skin.label, GUILayout.Width(width / 2));
						if (Event.current.type == EventType.Repaint)
						{
							thisStyle.Draw(rect, inactiveText, false, true, true, false);
						}
						GUILayout.EndHorizontal();
					};
