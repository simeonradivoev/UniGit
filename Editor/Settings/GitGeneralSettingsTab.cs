	...
	public class GitGeneralSettingsTab : GitSettingsTab, IHasCustomMenu
	{
		...
		internal override void OnGUI()
		{
			...
			if (gitSettings != null)
			{
				...
				var threadingContent = GitGUI.GetTempContent("Use Threading", "When Should Threading be used. In staging, unstaging or status retrival.");
				...
				var detectRenamesContent = GitGUI.GetTempContent("Detect Renames", "Detect Renames. This will make UniGit detect rename changes of files. Note that this feature is not always working as expected do the the modular updating and how Git itself works.");
			...
			var currentConfigFolder = gitManager.GetCurrentDotGitFolder();
            if (Directory.Exists(currentConfigFolder))
            {
                using var c = Configuration.BuildFrom(currentConfigFolder);
                ...
                var oldPath = c.GetValueOrDefault<string>("http.sslCAInfo");
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    var newPath = EditorUtility.OpenFilePanelWithFilters("Certificate", string.IsNullOrEmpty(oldPath) ? Application.dataPath : Path.GetFullPath(oldPath), new string[] { "", "cer", "", "pom", "", "crt" });
                    if (oldPath != newPath)
                    {
